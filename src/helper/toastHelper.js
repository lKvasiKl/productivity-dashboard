...
class ToastNotifications {
    ...
    showSuccess({ title = '', text = '' } = {}) {
        ...
        messageNode.classList.add('toast__message_type_success');
        this.showMessageNode(messageNode);
    }

    showInfo({ title = '', text = '' } = {}) {
        const messageNode = this.getMessageNode({ title, text });
        messageNode.classList.add('toast__message_type_info')
        this.showMessageNode(messageNode);
    }

    showWarning({ title = '', text = '' } = {}) {
        const messageNode = this.getMessageNode({ title, text });
        messageNode.classList.add('toast__message_type_warning')
        this.showMessageNode(messageNode);
    }

    showError({ title = '', text = '' } = {}) {
        const messageNode = this.getMessageNode({ title, text });
        messageNode.classList.add('toast__message_type_error')
        this.showMessageNode(messageNode);
    }
}
